class Enroll < ApplicationRecord
  ...
  validates :start_date, :timeliness => {:on_or_before => Date.today.beginning_of_day , :on_or_before_message => "must be less then or equal current date. (#{Date.today})"}   
  ...
  def active?
      ...
      if Date.today.between?(enroll.start_date, enroll.end_date.nil? ? Date.today.end_of_day : enroll.end_date)
        return true
      else
        return false   
      end      
